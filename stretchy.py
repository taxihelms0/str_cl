...
def user_quit(input):
    # function that watches input field for quit commands 'q' or 'quit'
    if '.wav' in input:
        pass
    elif 'q' in input or 'quit' in input or 'x' in input or 'exit' in input:
        print("bye")
        sys.exit(0)
...
def yesorno_query(param, message):
    ...
    while True:
        ...
        try:
            if len(query) == 1 and 'y' in query:
                query = True
                print(param)
                return query
                break
            elif len(query) == 1 and 'n' in query:
                query = False
                print("not " + param)
                return query
                break
...
while True:
    ...
    while True:
        infile = input("select a file to stretch: ")
        user_quit(infile)
        infile = './wavs/' + infile
        try:
    ...
    if randquery == True:
        ...
        bufsize = random.uniform(0.000, 5.000)
    ...
    if writeval == True:
        ...
        savequery = yesorno_query("save", "Would you like to save to a new wav file?")
        if savequery == True:
